...
| **J043 김석제** | 하루 종일 미션에 붙어 있다 보니 눈도 아프고 피곤해서, 코드와 잠시 떨어져 있는 활동이 필요했습니다. 휴식 같은 퀘스트를 하고 싶어 선택했습니다.                  | 할일은 부스트캠프에서 많이 주니까, 노선을 감장적 위로(F)로 정했습니다. AI에게 위로받는게 어떤건지 예시를 찾아봤는데, 주목할 부분이 있었다. [AI가 해주는 위로](https://www.youtube.com/watch?v=VRzT2nX1wkM) AI에게 공감을 바란다는게, 어떤 역할놀이를 하는게 아니다. 물론 이 역할놀이가 스트레스를 풀어주는 기능이 있다고 한다. 그러나 이런 역할놀이는 지시에 가깝고, 공감이라는것은 내 상황이 어떻구나 하고 인정하는걸 의미한다. 그래서 부스트캠프에서 경험하는 열등감이라던가, 힘듬, 고충과 성취감등을 AI에게 표현하면, 어떤 반응이 있을지 궁금했다. 결과는 생각보다 별로였다. 영상에서처럼 적극적으로 감정적 지지나 공감이 아닌, 해결방법을 제시하는 경향이 컸다. 이게 나쁘지는 않지만, 처음부터 원하는 반응이 오지는 않아서, 이번에는 내가 원하는게 공감이라는걸 알리고 반응을 살폈다. 그랬더나 반응이 너무 작위적이다. 사용자의 의사를 적극적으로 반영한다는건 보이지만, 뭔가 먹이는것 같은 위로라 이미 F 방식의 위로는 잘 안되는 것 같다. 지금까지는 Gemini였고, 이번에는 GPT로 넘어가서 반응을 살펴보았다. GPT는 무료버전임에도 유료인 GPT 보다는 훨씬 좋은 공감을 보여준다. 처음에는 상황을 말하면, 해결책을 제시했는데 공감을 원한다고 하니가 적절한 선에서 이해와 위로 순으로 말을 이어나간다. 보니까 Generative 모델마다 특정 작업 성능이 다른데, 공감 부문에서는 GPT가 앞서는것을 보인다. |
...
| 참가자          | 선택 이유                                                                                                  | 수행 내용 및 느낀 점                                                                                                                                                       |
| ------------ | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **J085 김효인** | 야크털을 깎다 보면 매일의 퀘스트를 전부 완료하지 못하는 경우가 많았습니다. 정해진 시간 제한과 분할된 일정이 있다면 좀 더 효율적으로 집중하고 구현할 수 있을 것 같아 선택했습니다. |  |
